  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch(
        "https://functions.poehali.dev/3a07f334-23a1-47ed-931c-c00deaa3ff1f",
        {
          method: "POST",
          headers: {
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center text-muted-foreground">
            ...
            <p className="text-xs mt-2">
              Например: «Найди разработчиков для ИИ проекта»
            </p>
          </div>
        </div>
      ) : (
              ...
              <div key={index} className="w-full">
                {message.role === "user" ? (
                  ...
                  <div className="w-full">
                    ...
                    {message.related_users_ids &&
                      message.related_users_ids.length > 0 && (
                        <button
                          onClick={() => {
      ...
      <div className="border-t p-4">
        <div className="relative max-w-3xl mx-auto">
          <div className="relative flex items-end bg-[#2f2f2f] rounded-3xl">
            ...
            >
              <Icon name="ArrowUp" size={18} className="stroke-2" />
            </button>
          </div>
        </div>
      </div>
